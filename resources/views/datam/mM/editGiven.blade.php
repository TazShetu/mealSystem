                    ...
                    <div class="col-lg-8 col-md-10 mb-5">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="h6 text-uppercase mb-0">Edit Given Meal Data</h3>
                            </div>
                            <div class="card-body">
                                {{--<p class="text-muted">Edit Meal Data</p>--}}
                                <form action="{{route('datam.mM.update.given', ['uid' => $u->id, 'msid' => $data->mealsystem_id, 'm' => $data->month, 'd' => $data->day])}}" method="post">
                                    @csrf
                                    <div class="form-group {{$errors->has('date') ? 'has-error' : ''}}">
                                        <label class="form-control-label text-uppercase"><b>Date</b></label>
                                        <p class="bg-light text-dark p-1">{{$d}} - {{$mn}}</p>
                                    </div>
                                    <div class="form-group {{$errors->has('name') ? 'has-error' : ''}}">
                                        <label class="form-control-label text-uppercase"><b>Name</b></label>
                                        <p class="bg-light text-dark p-1">{{$u->name}}</p>
                                    </div>
